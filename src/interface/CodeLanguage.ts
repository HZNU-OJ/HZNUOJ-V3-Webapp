...
export function checkCodeFileExtension(
  language: CodeLanguage,
  filename: string,
): boolean {
  return codeLanguageExtensions[language].some((extension) =>
...
export const getDefaultCompileAndRunOptions = (
  ...
  Object.fromEntries(
    compileAndRunOptions[codeLanguage].map(({ name, defaultValue }) => [
      name,
      defaultValue,
    ]),
  );
...
export const filterValidCompileAndRunOptions = (
  ...
  Object.assign(
    ...
    Object.fromEntries(
      Object.entries(inputOptions || ({} as Record<string, unknown>)).filter(
        ([name, value]) => {
          const option = compileAndRunOptions[codeLanguage].find(
